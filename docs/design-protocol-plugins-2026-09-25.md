...
执行形态、先后顺序及能力验收见 §16.2–§16.4，不将路线图中的语言标成当前已支持。
...
首版将选定的 Go 插件实现作为 package，与协议 runner **在构建阶段编译进同一可执行文件**。
...
新增或修改 Go 插件代码需要重新构建并部署该组 runner，再按 §8.4 重启整组；配置更新仍只重装目标实例。
这保留了“主进程不重编”的独立发布能力，但不承诺向现有 Go runner 动态加入任意新插件代码。
...
- 后续语言按 §16.4 明示 runner 的已实现能力；缺少流或反向调用时在装载前拒绝相应契约，不静默降级。
...
| L1–L3 语言扩展 | §16.2 的系统自动化 runner 和候选语言 | 按单语言能力验收，不阻塞已通过的语言发布 |
...
| --- | --- | --- | --- |
| L0 基础 | Node.js / TS、Python、Go、Rust | 共享语言进程；Go 按组构建，Rust 见 §十二 | 按 M0–M5 验证统一协议、依赖图和组恢复 |
