...
5. 增加最小 diagnostics DTO 与 apply 阶段的 ServiceAccess 越界记录。复用固定声明、实际绑定和已观测的 check 状态，读取快照不执行用户回调。与 #13 共享这一基础，完整 watch/观测扩展不在本项。
...
- [ ] 声明依赖中的越界与实际 apply 越界读取均有诊断，不泄露外部 provider 信息。
...
- [ ] Weak children 不因读取诊断、创建后失败或普通 dispose 留下历史条目。
- [ ] 原 contract/parity/config_update 流程通过，test/clippy/fmt 按设计文档执行并记录结果。

## 关联与交付

关联 https://github.com/arcships/rutis/issues/13。交付上游代码、流程测试、rustdoc；消费仓库的 vendor 回灌与来源记录由其迁移任务负责。
