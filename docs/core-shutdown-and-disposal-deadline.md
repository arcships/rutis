...
`dispose()` 保持原有的无限等待及 root 可重启语义。新增 `FiberView::dispose_with_timeout(limit)`：它同步登记同一个 dispose 任务，只给调用方等待设置截止时间。超时结果包含 plugin id、generation、当前状态和已等待时间；`Loading` 表示 apply 尚未退出，`Unloading` 表示清理或依赖消费者级联仍在进行。超时后驱动继续工作，重复 `dispose()` 会 join 原任务，最终清理错误沿用同一个 `Arc<CordisError>`。`timeout` 不宣称插件已停止，也不改变服务图状态。
