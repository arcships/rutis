...
async fn unknown_service_and_method_are_classified_remote_errors() {
    ...
    for (params, code) in [
        (
            json!({ "service": "nope", "method": "stream" }),
            "noService",
        ),
        (
            json!({ "service": "llm", "method": "generate" }),
            "unhandled",
        ),
    ] {
        host.wire
...
async fn hello_caps_derive_from_the_registry() {
    ...
    impl rutis_cordis::CordisService for Echo {
        ...
        async fn call(
            ...
            Err(rutis_cordis::RemoteError {
                code: "unhandled".into(),
                message: format!("echo/{method}"),
            })
        }
    }
    ...
    assert_eq!(
        dispatch.names(),
        vec!["echo".to_string(), "llm".to_string()]
    );
}
...
impl aimux_llm::LlmService for FakeLlm {
    async fn stream(&self, _req: aimux_llm::StreamRequest) -> Result<PartStream, LlmServiceError> {
    ...
    async fn list_models(
        ...
        Ok(vec![aimux_llm::ModelBrief {
            id: "mock-1".into(),
            owned_by: Some("rutis".into()),
            created: Some(1),
        }])
    }
}
...
async fn llm_face_list_models_and_illegal_stream_params_are_handled() {
    ...
    let reply = match face.call("stream", json!({})).await {
        Ok(r) => r,
        Err(e) => panic!(
            "empty stream params use serde(default), should succeed: {:?}",
            e
        ),
    };
    ...
    let err = match face
        .call(
            "stream",
            json!({ "options": { "messages": "not-an-array" } }),
        )
        .await
    {
        Err(e) => e,
    ...
    assert!(
        !err.code.is_empty(),
        "error code should be non-empty, got {:?}",
        err
    );
