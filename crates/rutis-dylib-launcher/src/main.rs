...
fn run() -> Result<(), String> {
    let host_name =
        option_env!("RUTIS_BUNDLE_HOST_FILE").ok_or("launcher was not bound to a host artifact")?;
    let host_hash =
        option_env!("RUTIS_BUNDLE_HOST_SHA256").ok_or("launcher was not bound to a host hash")?;
    let sdk_name =
        option_env!("RUTIS_BUNDLE_SDK_FILE").ok_or("launcher was not bound to an SDK artifact")?;
    let sdk_hash =
        option_env!("RUTIS_BUNDLE_SDK_SHA256").ok_or("launcher was not bound to an SDK hash")?;
    let std_name =
        option_env!("RUTIS_BUNDLE_STD_FILE").ok_or("launcher was not bound to libstd")?;
    let std_hash =
        option_env!("RUTIS_BUNDLE_STD_SHA256").ok_or("launcher was not bound to a libstd hash")?;
    ...
    for (name, _) in env::vars_os() {
        if name.to_string_lossy().starts_with("LD_") {
