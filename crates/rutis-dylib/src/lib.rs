...
impl Loader {
    ...
    pub unsafe fn load(&self, dir: impl AsRef<Path>) -> Result<Arc<Module>, LoadError> {
        ...
        if let Some(existing) = retained
            .iter()
            .find(|entry| entry.id == *id && entry.hash == actual_hash)
        ...
        fs::create_dir_all(cached.parent().unwrap()).map_err(|e| fail("cache", e.to_string()))?;
        if !cached.exists() {
            fs::write(&cached, &bytes).map_err(|e| fail("cache", e.to_string()))?;
        }
        if sha_file(&cached).map_err(|e| fail("cache", e))? != actual_hash {
            return Err(fail("cache", "cached library SHA-256 mismatch".into()));
        }
        let handle = open_library(&cached).map_err(|e| fail("dlopen", e))?;
        // Every successful dlopen is retained even if metadata or entry fails.
        ...
        retained.last_mut().unwrap().module = Some(module.clone());
        Ok(module)
    }
